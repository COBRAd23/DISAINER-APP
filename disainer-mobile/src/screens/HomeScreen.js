                ...
                notifications.map((notif) => (
                  <View key={notif.id} style={styles.notificationItem}>
                    ...
                    <View style={styles.notificationContent}>
                      ...
                      <Text style={styles.notificationTime}>{new Date(notif.date).toLocaleDateString()} {new Date(notif.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</Text>
                    </View>
                  </View>
                ))
              )}
...
const getStyles = ({ COLORS, TYPOGRAPHY, SPACING, isDark }) => StyleSheet.create({
  ...
  badge: {
    ...
    top: -5,
    right: -5,
    backgroundColor: COLORS.error || '#FF4B4B',
    width: 18,
    height: 18,
    borderRadius: 9,
    justifyContent: 'center',
